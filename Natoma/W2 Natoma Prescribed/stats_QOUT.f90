!  programs calcs outflow stats
      
      parameter(nstart=1)
      parameter(ih=650000,ns=1)
      REAL JDAY
	...
	real temp1(39),nh4n,nox
      ...
      OPEN(UNIT=10,FILE='qwo_str1_seg47.csv',status='old')
	do i=1,3
	read(10,*)
	end do
...
!c  subroutine reads data files
      ...
      common /sampdata /daydatacnt,wlqdata,ndata
          ...
          open(25,file='qot_br1.csv',status='old')	  
...
23      read(25,*,end=200)daydatacnt(nit),qout
           wlqdata(nit,1)=qout
...
!	  7 : ph
	...
	open(99,file='stats_QOUT.csv',status='unknown')
	write(99,344)
